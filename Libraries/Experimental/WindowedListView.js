...
type Props = {
   ...
   */
  renderFooter?: (showFooter: boolean) => ?React.Element<any>,
  /**
...
class WindowedListView extends React.Component<Props, State> {
  ...
  shouldComponentUpdate(newProps: Props, newState: State): boolean {
    ...
    const newDataSubset = newProps.data.slice(
      newState.firstRow,
      newState.lastRow + 1,
    );
    const prevDataSubset = this.props.data.slice(
      this.state.firstRow,
      this.state.lastRow + 1,
    );
    if (newDataSubset.length !== prevDataSubset.length) {
      DEBUG &&
        infoLog('  yes, subset length: ', {
          newLen: newDataSubset.length,
          oldLen: prevDataSubset.length,
        });
      return true;
    }
    for (let idx = 0; idx < newDataSubset.length; idx++) {
      if (
        newDataSubset[idx].rowData !== prevDataSubset[idx].rowData ||
        newDataSubset[idx].rowKey !== prevDataSubset[idx].rowKey
      ) {
        DEBUG &&
          infoLog('  yes, data change: ', {
            idx,
            new: newDataSubset[idx],
            old: prevDataSubset[idx],
          });
        return true;
      }
    }
  ...
  _onScroll = (e: Object) => {
    ...
    if (
      this.props.onViewableRowsChanged &&
      Object.keys(this._rowFrames).length
    ) {
    ...
    if (DEBUG) {
      ...
      if (this._rowFrames[rowKey]) {
        ...
        if (deltaY > 2 || deltaH > 2) {
          const dataEntry = this.props.data.find(
            datum => datum.rowKey === rowKey,
          );
          console.warn('layout jump: ', {
            dataEntry,
            prevLayout: this._rowFrames[rowKey],
            newLayout: layout,
          });
        }
      }
  ...
  _computeRowsToRender(props: Object): void {
    ...
    const rowsShouldChange =
      firstRow !== this.state.firstRow || lastRow !== this.state.lastRow;
    if (this._rowFramesDirty || rowsShouldChange) {
      if (rowsShouldChange) {
        ...
        infoLog('WLV: row render range will change:', {
          firstRow,
          firstVis: this._firstVisible,
          lastVis: this._lastVisible,
          lastRow,
        });
      }
  ...
  render(): React.Node {
    ...
    const firstBuffer =
      firstRow === 0 ? Infinity : this._firstVisible - firstRow;
    const lastBuffer =
      lastRow === this.props.data.length - 1
        ? Infinity
        : lastRow - this._lastVisible;
    const minBuffer = Math.min(firstBuffer, lastBuffer);
    const disableIncrementalRendering =
      this.props.disableIncrementalRendering ||
    ...
    for (let ii = firstRow; ii <= lastRow; ii++) {
      ...
      if (
        rowFrames[rowKey].offscreenLayoutDone ||
        this._rowRenderMode[rowKey] === 'sync'
      ) {
        if (ii > 0) {
    ...
    for (let idx = firstRow; idx <= lastRow; idx++) {
      ...
      const includeInLayout =
        this._rowRenderMode[rowKey] === 'sync' ||
        (this._rowFrames[rowKey] &&
          this._rowFrames[rowKey].offscreenLayoutDone);
      rows.push(
        <CellRenderer
          key={rowKey}
          rowKey={rowKey}
    ...
    return this.props.renderScrollComponent({
...
class CellRenderer extends React.Component<CellProps> {
  ...
  UNSAFE_componentWillMount() {
    if (this.props.asyncRowPerfEventName) {
      ...
      infoLog(
        // $FlowFixMe(>=0.28.0)
        `perf_asynctest_${this.props.asyncRowPerfEventName}_start ${
          this._perfUpdateID
        } ` + `${Date.now()}`,
      );
    }
  ...
  _updateParent() {
    ...
    if (this.props.asyncRowPerfEventName) {
      ...
      infoLog(
        // $FlowFixMe(>=0.28.0)
        `perf_asynctest_${this.props.asyncRowPerfEventName}_end ${
          this._perfUpdateID
        } ` + `${Date.now()}`,
      );
    }
  ...
  UNSAFE_componentWillReceiveProps(newProps) {
    if (newProps.includeInLayout && !this.props.includeInLayout) {
      invariant(
        this._offscreenRenderDone,
        'Should never try to add to layout before render done',
      );
  ...
  render() {
    ...
    return (
      <IncrementalGroup
        ...
        <View ref={this._setRef} style={style} onLayout={this._onLayout}>
          {debug}

...
QuickJSRuntime::QuickJSRuntime(const std::string &codeCacheDir, int maxStackSize) {
  runtime_ = JS_NewRuntime();
  if (maxStackSize >= 0) {
    JS_SetMaxStackSize(runtime_, maxStackSize);
  }
...
jsi::Value QuickJSRuntime::lockWeakObject(jsi::WeakObject &weakObject) {
  // const QuickJSPointerValue *pointer =
  //     (const QuickJSPointerValue *)(getPointerValue(weakObject));
  // JSValue value = pointer->Get(context_);
  // ScopedJSValue scopedJsValue(context_, &value);
